...
{
    public partial class Perfumeria : System.Web.UI.Page
    {
        ...
        protected void ImageButtonARABEAMARILLO_Click(object sender, ImageClickEventArgs e)
        {
        ...
        protected void TextBoxARABEAMARILLO_TextChanged(object sender, System.EventArgs e)
        {

        }

        protected void btnARABEAMARILLO_Click2(object sender, EventArgs e)
        {
            Button btn = (Button)sender;
            string producto = btn.CommandArgument;

            Control contenedor = btn.NamingContainer;
            TextBox TextBoxARABEAMARILLO = null;

            foreach (Control ctrl in contenedor.Controls)
            {
                if (ctrl is TextBox)
                {
                    TextBoxARABEAMARILLO = (TextBox)ctrl;
                    break;
                }
            }

            int cantidad = 1;
            if (TextBoxARABEAMARILLO != null)
            {
                int.TryParse(TextBoxARABEAMARILLO.Text, out cantidad);
            }
